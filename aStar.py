...
def fill_shortest_path(board, start, end, max_distance=math.inf):
    ...
    nboard.at(start).mark = Game.CellMark.Start
    nboard.at(end).mark = Game.CellMark.End
    ...
    while open_list:
        ...
        for neighbour in neighbours:
            ncell_pos = Game.add_point(cur_pos, neighbour)
            if not nboard.is_valid_point(ncell_pos):
                continue

            cell = nboard.at(ncell_pos)

            if cell.type != Game.CellType.Empty:
                continue
